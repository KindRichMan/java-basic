...
-> set as default schema를 클릭하세요. 이러면 현재 명령을 받을 DB로 저장됩니다.*/
...
CREATE TABLE users(
     ...
     INSERT INTO users(id, pw, user_name, email) values
     ('abc1234', 'asdf', '김자바', 'asdf@asdf.com');user
      INSERT INTO users values('qwer123', 'qwer123', '쿼리문', 'query!query.com');
     
     -- 한 줄 주석은 자바와 달리 -- 내용 을 적어 작성할 수 있습니다.
     -- 여러분들이 계정 3개를 더 넣어주시되, 1개 계정은 이메일을 생략하고 넣어주세요.
     -- 이메일 없이 넣는 경우는 컬럼명을 반드시 표기해야합니다.
      INSERT INTO users(id, pw, user_name) values('abdc1234', 'afsdf', '김메자바');
    ...
    /* 데이터 조회하기
     ...
     -- 조회구문에 조건으 걸기 위해서 WHERE이라는 구문을 사용합니다.
     ...
     -- 조건 여러개를 걸 경우 AND, OR키워드를 쓰면 됩니다.alter
     ...
     UPDATE users SET pw='pppp'  WHERE id='adc1234';
     ...
     -- 데이터 삭제alter
     /* DELETE FROM 테이블명; 을 할 경우 테이블 전체 데이터가 싹 다 날아갑ㄴ.다.
     DELETE구문 역시 WHERE과 조합해서 사용해야 합니다.*/
     DELETE FROM users WHERE id='adc31234';
     ...
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('이건희에세이', 500000, '무한출판사', '이건희',230);
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('초격차', 20000, '무달출판사', '권오현',300);
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('한비자', 30000, '한비출판사', '한비자',400);
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('도덕경', 10000, '노자출판사', '최진석',300);
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('인간의길', 34000, '사마천출판사', '김영수',320);
     INSERT INTO library(book_name, book_price, book_pub, book_writer,book_page) VALUES ('최진석의 대한민국 읽기', 45000, '최씨출판사', '최진석',520);
